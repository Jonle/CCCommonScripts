...
export class ListView {
    ...
    constructor(params: ListViewParams) {
        ...
        this.item_anchorX = this.item_tpl.data.anchorX;
        this.item_anchorY = this.item_tpl.data.anchorY;
        ...
        if (this.dir == ListViewDir.Vertical) {
            ...
            if (real_width > this.width) {
                console.info("real width > width, resize scrollview to realwidth,", this.width, "->", real_width);
            ...
            if (real_height > this.height) {
                console.info("real height > height, resize scrollview to realheight,", this.height, "->", real_height);
        ...
        // console.info("constructor", this.mask.width, this.mask.height, this.scrollview.node.width, this.scrollview.node.height, this.content.width, this.content.height);
    }
    ...
    private on_scrolling() {
        ...
        if (this.dir == ListViewDir.Vertical) {
            ...
            // console.info("onscrolling, content posy=", posy);
            if (posy < 0) {
                posy = 0;
            }
            let gapY: number = this.content.height - this.height;
            if (posy > gapY) {
                posy = gapY;
            }
            let start: number = 0;
            ...
            let item_gapHeight: number = this.item_height*this.item_anchorY;
            while (this.items[start].y - item_gapHeight > viewport_start) {
                start++;
            }
            ...
            if (start != this.start_index && stop != this.stop_index) {
                ...
                // console.info("render_from:", start, stop);
                this.render_items();
            }
        }
        else {
            let posx: number = this.content.x;
            // console.info("onscrolling, content posx=", posx);
            if (posx > 0) {
                posx = 0;
            }
            let gapX: number = this.width - this.content.width;
            if (posx < gapX) {
                posx = gapX;
            }
            let start: number = 0;
    ...
    private layout_items(start: number) {
        // console.info("layout_items, start=", start);
    ...
    private resize_content() {
        ...
        // console.info("resize_content", this.mask.width, this.mask.height, this.scrollview.node.width, this.scrollview.node.height, this.content.width, this.content.height);
    }
    ...
    scroll_to(index: number) {
        if (this.dir == ListViewDir.Vertical) {
            ...
            if (x > max_x) {
                x = max_x;
                console.info("content reach right");
            }
            if (x < 0) {
                x = 0;
                console.info("content reach left");
            }
